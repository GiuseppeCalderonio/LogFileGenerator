...
import HelperUtils.Parameters.ConfigType2Process
...
object Parameters:
  ...
  val config: Config = ObtainConfigReference("randomLogGenerator") match {
  ...
  private def logMsgRange(logTypeName: String): (Double, Double) =
    val lst = Try(config.getDoubleList(s"randomLogGenerator.logMessageType.$logTypeName").asScala.toList) match {
      case Success(value) => value.sorted
      case Failure(_) => logger.error(s"No config parameter is provided: $logTypeName")
  ...
  private def getParam[T](pName: String, defaultVal: T): ConfigType2Process[T] =
    defaultVal match {
      ...
      case _: (Double, Double) => logMsgRange(pName)
  ...
  val awsBucket: ConfigType2Process[String] = getParam("awsBucket", "peppe-bucket-test")
  val awsKey: ConfigType2Process[String] = getParam("awsKey", "input.log")
  ...
  //if two types of messages have the same likelihood of being generated, then this
  ...
  private def checkForNestedRanges(input: ListMap[(Double, Double), _]): Boolean =
    val overlaps = for {
      range1 <- input.keySet
      range2 <- input.keySet
  ...
  private val compFunc: ((Double, Double), (Double, Double)) => Boolean = (input1, input2) => (input1._1 <= input2._1) && (input1._2 < input2._2)
    val logRanges = ListMap(Map[(Double, Double), String => Unit](
  ...
  if checkForNestedRanges(logRanges) then throw new Exception("Overlapping likelihood ranges will lead to the loss of precision.")
