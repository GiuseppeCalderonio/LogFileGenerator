...
case class RandomStringGenerator(lengthRange: (Int, Int), seed: Long) {
  ...
  def  next: (RandomStringGenerator, String) =
...
object RSGStateMachine {
  type RSGFunction = RandomStringGenerator => (RandomStringGenerator, String)

  def unit(): RSGFunction = rsg => rsg.next
